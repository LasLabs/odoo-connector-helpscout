...
class HelpScoutImporter(AbstractComponent):
    """ Base importer for HelpScout """

    _name = 'helpscout.importer'
    _inherit = ['base.importer', 'base.helpscout.connector']
    _usage = 'record.importer'

    def __init__(self, work_context):
        super(HelpScoutImporter, self).__init__(work_context)
        self.external_id = None
        self.helpscout_record = None

    def _get_helpscout_data(self):
        """Return the raw HelpScout data for ``self.external_id``."""
        return self.backend_adapter.read(self.external_id)

    def _before_import(self):
        """Hook called before the import, when we have the HelpScout data."""
        return

    def _is_up_to_date(self, binding):
        """Return True if the data is already up to date in Odoo."""
        assert self.helpscout_record
        if not hasattr(self.helpscout_record, 'modified_at'):
            return  # no update date on HelpScout, always import it.
        if not binding:
        ...
        helpscout_date = self.helpscout_record.modified_at
        # if the last synchronization date is greater than the last
        # update in helpscout, we skip the import.
        # Important: at the beginning of the exporters flows, we have to
        # check if the helpscout_date is more recent than the sync_date
        # and if so, schedule a new import. If we don't do that, we'll
        # miss changes done in HelpScout.
        return helpscout_date < sync_date
    ...
    def _map_data(self):
        ...
        """
        return self.mapper.map_record(self.helpscout_record)
    ...
    def _create(self, data):
        ...
        _logger.debug(
            '%d created from helpscout %s', binding, self.external_id,
        )
        return binding
    ...
    def _update(self, binding, data):
        ...
        _logger.debug(
            '%d updated from helpscout %s', binding, self.external_id,
        )
        return
    ...
    def run(self, external_id, force=False, external_record=None):
        ...
        Args:
            external_id (int | helpscout.BaseModel): identifier of the
                record in HelpScout, or a HelpScout record.
            force (bool, optional): Set to ``True`` to force the sync.
            external_record (helpscout.models.BaseModel): Record from
        ...
        if external_record is not None:
            self.helpscout_record = external_record
        else:
            try:
                self.helpscout_record = self._get_helpscout_data()
            except IDMissingInBackend:
                return _('Record no longer exists in HelpScout.')
...
class BatchImporter(AbstractComponent):
    ...
    _name = 'helpscout.batch.importer'
    _inherit = ['base.importer', 'base.helpscout.connector']
...
class DirectBatchImporter(AbstractComponent):
    ...
    _name = 'helpscout.direct.batch.importer'
    _inherit = 'helpscout.batch.importer'
...
class DelayedBatchImporter(AbstractComponent):
    ...
    _name = 'helpscout.delayed.batch.importer'
    _inherit = 'helpscout.batch.importer'
