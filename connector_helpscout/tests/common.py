...
class HelpScoutTestCase(SavepointComponentCase):
    """ Base class - Test the imports from a HelpScout Mock.

    The data returned by HelpScout are those created for the
    demo version of HelpScout on a standard 1.9 version.
    """

    def setUp(self):
        super(HelpScoutTestCase, self).setUp()
        # disable commits when run from pytest/nosetest
        odoo.tools.config['test_enable'] = True

        self.backend_model = self.env['helpscout.backend']
    ...
    def get_helpscout_helper(self, model_name):
        return HelpScoutHelper(self.cr, self.registry, model_name)
    ...
    def _import_record(self, model_name, helpscout_id, cassette=True):
        assert model_name.startswith('helpscout.')
        table_name = model_name.replace('.', '_')
        # strip 'helpscout_' from the model_name to shorted the filename
        filename = 'import_%s_%s' % (table_name[8:], str(helpscout_id))

        def run_import():
            with mock_urlopen_image():
                self.env[model_name].import_record(self.backend, helpscout_id)
...
class HelpScoutSyncTestCase(HelpScoutTestCase):

    def setUp(self):
        super(HelpScoutSyncTestCase, self).setUp()
